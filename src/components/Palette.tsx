...
const APPS: { label: string; to: string; hint: string }[] = [
  ...
  { label: 'Lab', to: '/lab', hint: 'datasets, experiments, curves' },
  { label: 'Docs', to: '/docs', hint: 'specs, research, sheets, records' },
  { label: 'Connections', to: '/connections', hint: 'what the OS can reach' },
...
export default function Palette() {
  ...
  const hits = useMemo<Hit[]>(() => {
    ...
    const all: Hit[] = [
      ...
      ...initiatives.map((i) => ({ id: `in-${i.id}`, group: 'Initiatives' as const, label: i.title, hint: (i.workflowId ?? '').replace('wf-', ''), run: () => go(`/initiatives?id=${i.id}`) })),
      { id: 'act-workflow', group: 'Actions', label: 'Design a workflow', hint: 'assemble stages into a new one', run: () => go('/initiatives?tab=workflows') },
